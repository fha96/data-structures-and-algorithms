...
const addExclamation = (arr) => {
  // Solution code here...
  const newArray=[];
  arr.forEach(addString =>{
    newArray.push(addString+"!")
  });
  return newArray;
};
...
const createList = (availableItems) => {
  // Solution code here...
  const groceryList=[];
  availableItems.forEach(check => {
    if(check.available){
      groceryList.push(check.name);
    }} )
  return groceryList; 
};
